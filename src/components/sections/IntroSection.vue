<template>
  <div class="section-intro">
    ...
    <div class="text-content">
      ...
      <p>
        週末我經常會獨自或與少數幾位好友一起走進大自然，或是來一杯手沖咖啡，喝一杯香醇的咖啡是我放鬆與享受的方式，也很期待未來和你一起分享我的喜悅。閒暇時我也喜歡與長者們分享知識與經驗，這讓我感到無比的滿足。能夠為他們帶來歡樂與新的學習機會，是我最有成就感的事情之一。
      </p>
      <div class="buttons">
      ...
      <CustomModal ref="adventureModal">
        <div class="adventure-content">
          <div class="text-part">
            ...
            <p>
              爬山不僅讓我有機會鍛鍊身體，更是一次與自己對話的時光。當我漫步在山林間，聆聽鳥鳴和微風拂過樹梢的聲音時，那種寧靜與自由的感覺無可比擬。我喜歡記錄下這些瞬間，透過影像和文字與朋友分享，讓大家也能感受到這份簡單而純粹的喜悅。
            </p>
...
const adventureModal = ref(null);
const coffeeModal = ref(null);
const teachingModal = ref(null);
const coffeeVideo = ref(null);
const teachingVideo = ref(null);

const openModal = (type) => {
  if (type === 'adventure') {
    adventureModal.value.openModal();
  } else if (type === 'coffee') {
    coffeeModal.value.openModal();
    nextTick(() => {
      if (coffeeVideo.value) {
        coffeeVideo.value.play();
      }
    });
  } else if (type === 'teaching') {
    teachingModal.value.openModal();
    nextTick(() => {
      if (teachingVideo.value) {
        teachingVideo.value.play();
      }
    });
  }
};
